<?php  
    ...
    if (is_single()) {
        ...
        $nz_rh_height           = (isset( $values['rh_height'][0]) && !empty($values['rh_height'][0])) ? $values["rh_height"][0] : "300";
        ...
        $nz_rh_height           = (isset($nz_ninzio['port-rh-height']) && $nz_ninzio['port-rh-height']) ? esc_attr($nz_ninzio["port-rh-height"]) : "300";
...
<?php if ($nz_rh == "true"): ?>
    <header class="rich-header port-header" data-parallax="<?php echo $nz_parallax; ?>" style="<?php echo $nz_rh_styles; ?>">
        <?php if (is_single()){ninzio_post_nav($post->ID);} ?>
...
<?php if (!is_single() && !is_tax('portfolio-category') && $nz_filter == "true"): ?>
    ...
    <div class="nz-portfolio-filter portfolio-archive-filter">
        <span class="filter active" data-group="all"><?php echo __('Show All', TEMPNAME); ?></span>
