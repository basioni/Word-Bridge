...
/*================================*/
	...
	function nz_dropcap( $atts, $content = null ) {
		...
		$output = '<span '.$color.' class="nz-dropcap '.esc_attr($type).'">'.do_shortcode($content).'</span>';
		...
		$output = "<div id='$selector' class='nz-gallery galleryid-{$id}  gallery-size-{$size_class}' data-columns='".$columns."''>";
			...
			foreach ( $attachments as $id => $attachment ) {
				...
				if ( ! empty( $link ) && 'file' === $link ){
					$image_output = $before_img.'<a data-lightbox-gallery="gallery2" href="'.$img_full[0].'"><div class="ninzio-overlay"><div class="ovhover"></div></div><img alt="'.$attachment->post_excerpt.'" src="'.$image_output[0].'" width="'.$image_output[1].'" height="'.$image_output[2].'"></a>'.$after_img;
				}
	...
	function nz_sl($atts, $content = null) {
		...
		foreach($atts as $social => $href) {
			if($href && $social != 'target' && $social != 'align' && $social != 'link_color' && $social != 'link_back_color') {
				if ($social == "email") {
					$output .='<a class="icon-envelope" href="'.esc_url($href).'" target="'.esc_attr($target).'" ><span class="bubble">'.$social.'</span></a>';
				} else {
					$output .='<a class="icon-'.$social.'" href="'.esc_url($href).'" target="'.esc_attr($target).'" ><span class="bubble">'.$social.'</span></a>';
				}
			}
		}
...
/*================================*/
	...
	function nz_mailchimp($atts, $content = null) {
		...
		if (isset($action) && !empty($action) && isset($name) && !empty($name)) {
			...
			$output .='<div class="nz-mailchimp-wrap nz-clearfix">';

				$args = array(
					'before_widget' => '<div id="nz-mailchimp-'.$id_counter.'" class="nz-mailchimp">',
					'after_widget'  => '</div>'
				);
				...
				$output .= get_the_widget( 'WP_Widget_Mailchimp', $instance,$args);
...
/*================================*/
	...
	function nz_tagline_3($atts, $content = null) {
		...
		$output .= '<div id="nz-tagline-3-'.$id_counter.'">';
		$output .= '<style scoped>'; 
			$output .= '#nz-tagline-3-'.$id_counter.' a {'.esc_attr($styles).';}';
			$output .= '#nz-tagline-3-'.$id_counter.' a:hover {'.esc_attr($styles_hover).';}';
		$output .= '</style>';
		$output .= '<a href="'.esc_url($link).'" class="nz-tagline-3 nz-clearfix">';
			$output .='<div class="container">';
				if(isset($title) && !empty($title)){$output .= '<div class="tagline-title"><span class="title-content">'.esc_html($title).'</span></div>';}
				if(isset($icon) && !empty($icon)){$output .= '<div class="tagline-icon '.sanitize_html_class($icon).'"></div>';}
			$output .= '</div>';
		$output .= '</a></div>';
	...
	add_shortcode('nz_tagline_3', 'nz_tagline_3');
...
/*================================*/
	...
	function nz_timer($atts, $content = null) {
		...
		$output ='<div id="nz-timer-'.$id_counter.'" class="nz-timer-wrap"><div class="nz-timer nz-clearfix" '.$enddate.' '.$days.' '.$hours.' '.$minutes.' '.$seconds.'></div></div>';
	...
	function nz_circle($atts, $content = null) {
		...
		$output .= '<div class="nz-circle">';
			...
			$output .= '<div class="circle" '.$data_attr.' data-percent="'.absint($percentage).'">';
				$output .= '<span class="title" '.$color_styles.'><span class="nz-value">0</span><span>%</span></span>';
			$output .= '</div>';
...
/*================================*/
	...
	function nz_count($atts, $content = null) {
		...
		if(isset($count_color) && !empty($count_color)) {
			$count_styles = 'style="color:'.$count_color.';"';
		} else {
			$count_styles = 'style="color:'.$nz_color.';"';
		}
        ...
        $output .= '<div class="nz-count">';
        	if(isset($icon) && !empty($icon)) {
				$output .= '<span class="count-icon '.sanitize_html_class($icon).'" '.$icon_color.'></span>';
			}
			$output .= '<span '.$value.' '.$count_styles.' class="count-value">0</span>';
			if(isset($content) && !empty($content)) {
				$output .= '<div class="count-content">'.do_shortcode($content).'</div>';
			}
        $output .= '</div>';
		return $output;
	}
		...
		$output .= '<div id="nz-box-'.$id_counter.'" class="'.$extra_class.' nz-box">';
			$output .= $link_before;
				$output .= '<style>';
					...
					if(isset($icon_border_color) && !empty($icon_border_color)){
						$output .= '#nz-box-'.$id_counter.' .box-icon:after {box-shadow:inset 0 0 0 4px '.$icon_border_color.';}';
					}

					if (isset($icon_hover_effect_color) && !empty($icon_hover_effect_color)) {
						$output .= '#nz-box-'.$id_counter.' .box-icon-wrap:after {background-color:'.$icon_hover_effect_color.' !important;}';
					}
				$output .= '</style>';
				if(isset($icon) && !empty($icon)){
				...
				$output .= '<div class="person-wrap">';
					$output .= '<div class="person-inner">';
							...
							$output .= '<div class="person-body">';
								$output .= $link_before;
								$output .='<div class="img"><div class="ninzio-overlay"><div class="ovhover"></div></div><img src="'.esc_url($img).'" alt="'.esc_attr($name).'" /></div>';
								$output .= $link_after;
								$output .='<div class="person-meta">';

									if(isset($name) && !empty($name)){
										$output .= '<div class="name"><h4>'.esc_html($name).'</h4></div>';
									}
									if(isset($title) && !empty($title)){
...
/*================================*/
	...
	function nz_column($atts, $content = null) {
		...
		if (isset($color) && !empty($color)) {
				$styles.= '#nz-column-'.$id_counter.' .pricing {background-color:'.$color.';}';
			if ($high == "true") {
											...
											if (!empty($nz_image_url)) {
												$output .='<a class="nz-more" href="'.get_permalink().'">';
													$output .= '<div class="nz-thumbnail">';
														...
														$output .= '<div class="ninzio-overlay"><div class="ovhover"></div></div>';
					...
					$output .= '<div id="nz-recent-portfolio-'.$id_counter.'" data-animate="'.esc_attr($animate).'" data-autoplay="'.esc_attr($autoplay).'" data-columns="'.absint($columns).'" class="lazy nz-recent-portfolio '.$classes.'">';
						...
						$output .= '<div class="nz-portfolio-posts">';
							while($recent_portfolio->have_posts()) : $recent_portfolio->the_post();
								...
								$values = get_post_custom( $post->ID );
								$nz_url = ($ext_url == "true") ? (isset($values["project_link"][0]) ? $values["project_link"][0] : get_permalink()) : get_permalink();
										...
										$output .= '<div class="post-inner">';
											$output .= '<div class="post-body">';
												...
												$output .= '<div class="nz-thumbnail">';
													...
													$output .= '<div class="ninzio-overlay"><div class="ovhover">';

														if ($type_switch == "false") {
															$output .='<div class="overlay-content">';
																$output .='<h3 class="project-title">'.get_the_title().'</h3>';
																$output .= '<br>';
																$output .='<a class="button button-normal small round animate-false hover-fill" href="'.$nz_url.'">'.__('View project', TEMPNAME).'</a>';
															$output .= '</div>';
														} else {
															$output .='<a href="'.$nz_url.'"></a>';
														}
