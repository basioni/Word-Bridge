...
/**
 * Returns true if current page is the shopping page
 *
 * @since 1.2.5.2
 */
function tcp_is_the_shopping_cart_page() {
	return is_page( tcp_get_the_shopping_cart_page_id() );
}

/**
 * Returns the shopping page id
 *
 * @since 1.4.9
 */
function tcp_get_the_shopping_cart_page_id() {
	return tcp_get_current_id( get_option( 'tcp_shopping_cart_page_id' ), 'page' );
}

/**
 * Outputs or return the checkout url
 */
...
/**
 * Returns true if current page is the checkout page
 *
 * @since 1.2.5.2
 */
function tcp_is_the_checkout_page( $post_id = false ) {
	return is_page( tcp_get_the_checkout_page_id() );
}

/**
 * Returns the checkout page id
 *
 * @since 1.4.9
 */
function tcp_get_the_checkout_page_id() {
	return tcp_get_current_id( get_option( 'tcp_checkout_page_id' ), 'page' );
}

/**
 * Returns true if current page is the catalogue page
 *
 * @since 1.2.5.2
 */
function tcp_is_the_catalogue_page() {
	return is_page( tcp_get_the_catalogue_page_id() );
}
...
function tcp_the_my_account_url( $echo = true ) {
	$url = get_permalink( tcp_get_the_my_account_page_id() );
...
/**
 * Returns the My Account page id
 *
 * @since 1.4.9
 */
function tcp_get_the_my_account_page_id() {
	return tcp_get_current_id( get_option( 'tcp_my_account_page_id' ), 'page' );
}
...
//		'redirect'			=> 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'],
	...
	<form method="post">
		...
		<p>
			<label for="user_login"><?php _e( 'E-mail', 'tcp' ); ?>:</label>
...
<div id="tcp_login">
	<form id="<?php echo $args['form_id']; ?>" method="post" action="<?php echo $url; ?>" name="<?php echo $args['form_id']; ?>">
		...
		<div class="tcp_login_remember">
			...
			<label for="<?php echo esc_attr( $args['id_remember'] ); ?>" class="tcp_login_remember_label"><?php echo esc_html( $args['label_remember'] ); ?></label>
		</div>
...
<?php // User is logn in
	// Tries to displays lostpassword form
	} elseif ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'tcp-reset-pass' ) {
		$display_form = true;
		$errors = array();
		$current_user = wp_get_current_user();
		$user_id = isset( $_REQUEST['rp_login'] ) ? $_REQUEST['rp_login'] : false;
		if ( $user_id != false ) {
			// checks if the user is the current one
			if ( $user_id == $current_user->ID ) {
				$prev_pass = isset( $_REQUEST['prev-pass'] ) ? $_REQUEST['prev-pass'] : false;
				$pass1 = isset( $_REQUEST['pass1'] ) ? $_REQUEST['pass1'] : false;
				$pass2 = isset( $_REQUEST['pass2'] ) ? $_REQUEST['pass2'] : false;
	
				// Checks if passwords match
				if ( $pass1 !== false && $pass1 == $pass2 ) {
	
					// Checks if valid password
					if ($user_id == $current_user->ID && wp_check_password( $prev_pass, $current_user->data->user_pass, $current_user->ID ) ) {
						wp_set_password( $pass1, $current_user->ID ); ?>
						
						<div class="tcp-change-password-success">
							<?php _e( 'Password change successfully', 'tcp' ); ?>
						</div>
						<?php printf( __( 'Please, <a href="%s" class="btn btn-link">login</a> again', 'tcp' ), tcp_the_my_account_url( false ) ); ?>
						
						<?php $display_form = false;
					} else {
						$errors[] = __( 'You have entered an invalid password', 'tcp' );
					}
				} else {
					$errors[] = __( 'Passwords do not match!', 'tcp' );
				}
			} else {
				$errors[] = __( 'An error occurred', 'tcp' );
			}
		} ?>
...
<div id="password-reset-form" class="widecolumn">
    ...
    <form name="resetpassform" id="resetpassform" action="" method="post" autocomplete="off">
		<input type="hidden" name="action" value="tcp-reset-pass" />
		<input type="hidden" id="user_login" name="rp_login" value="<?php echo esc_attr( $current_user->ID ); ?>" autocomplete="off" />

		<?php if ( isset( $errors ) && count( $errors ) > 0 ) : ?>
			<?php foreach ( $errors as $error ) : ?>
				<p class="descripcion error"><?php echo $error; ?></p>
			<?php endforeach; ?>
		<?php endif; ?>
 
		<div class="tcp-prev-pass-label">
			<label for="pass1"><?php _e( 'Current password', 'tcp' ) ?></label>
		</div>
		
		<div class="tcp-prev-pass">
			<input type="password" name="prev-pass" id="prev-pass" class="input" size="20" value="" autocomplete="off" />
		</div>

		<div class="tcp-pass1-label">
		...
		<p class="resetpass-submit">
			<button type="submit" name="action" value="tcp-reset-pass" id="resetpass-button" class="button btn btn-default"><?php _e( 'Reset Password', 'tcp' ); ?></button>
		</p>
	</form>
</div>
<?php endif; ?>
...
//		'redirect'		=> 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'],
	...
	if ( ! is_user_logged_in() ) :
		...
		if ( isset( $_REQUEST['tcp_register_error'] ) ) : ?>
	...
	<form class="tcp_register" action="<?php echo $url ?>" method="post">
		...
		<?php else : ?>
	<form action="<?php echo $url ?>" method="post" role="form" class="form-horizontal tcp_register">
		<input type="hidden" name="action" value="tcp_register_and_login<?php if ( $args['ajax'] ) : ?>_ajax<?php endif; ?>" />

		<?php ob_start(); ?>
		<div class="form-group">
			<label for="tcp_new_user_name" class="col-sm-2 control-label"><?php echo $args['label_username']; ?></label>
			<div class="tcp_register_username col-sm-4">
				<input type="text" name="tcp_new_user_name" class="form-control" value="<?php echo isset( $_REQUEST['tcp_new_user_name'] ) ? $_REQUEST['tcp_new_user_name'] : ''; ?>" />
			</div>
		</div>

		<div class="form-group">
			<label for="tcp_new_user_pass" class="col-sm-2 control-label"><?php echo $args['label_password']; ?></label>
			<div class="tcp_register_password col-sm-4">
				<input type="password" name="tcp_new_user_pass" class="form-control input"/>
			</div>
		</div>

		<div class="form-group">
			<label for="tcp_repeat_user_pass" class="col-sm-2 control-label"><?php echo $args['label_repeat_password']; ?></label>
		
			<div class="tcp_register_repeat_password col-sm-4">
				<input type="password" name="tcp_repeat_user_pass" class="form-control input"/>
			</div>
		</div>

		<div class="form-group">
			<label for="tcp_user_email" class="col-sm-2 control-label"><?php _e( 'E-mail', 'tcp' ); ?></label>
		
			<div class="tcp_register_user_email col-sm-8">
				<input type="email" name="tcp_new_user_email" class="form-control" value="<?php echo isset( $_REQUEST['tcp_new_user_email'] ) ? $_REQUEST['tcp_new_user_email'] : ''; ?>" maxlength="100"/>
			</div>
		</div>
		<?php endif; ?>
		...
		<?php if ( !$use_custom_login_ver_two ) : ?>
		<p>
		...
		<p id="tcp_error_register" class="tcp_error warning" style="display:none;"><?php _e( 'Error', 'tcp' ); ?>: </p>
	</form>
