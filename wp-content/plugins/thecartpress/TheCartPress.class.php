...
class TheCartPress {
	...
	private function includes() {
		// Loads main tcp_ funcstions
		require_once( TCP_TEMPLATES_FOLDER			. 'manage_templates.php' );

		// Loads custom post type definitions
	...
	public function init() {
		...
		wp_enqueue_style( 'select2-style', plugins_url( 'css/select2.min.css', __FILE__ ) );
		...
		// Allows to add 'init' actions to TheCartPress plugins or modules (since 1.3.2)
		do_action( 'tcp_init', $this );
	}

	function last_init() {
		// Has anyone clicked on any Add button?
		$this->check_for_shopping_cart_actions();
	}

	/**
	 * Loads the translation file for current language. Checks the languages
	 ...
	 *
	 * @return ShoppingCart
	 * @uses tcp_session_start
	 * @uses apply_filters calling 'tcp_get_shooping_cart'
	 */
	...
	/**
	 * Removes all contetn of the shopping cart.
	 ...
	 * 
	 * @uses apply_filters calling 'tcp_check_the_plugin'
	 */
	...
	function pre_get_posts( $query ) {
		...
		if ( $query->is_author ) {
			...
			// Applies saleable filters if the current page has been injected by ThemeCompatibility
		...
		if ( !$apply_filters && isset( $query->tax_query ) ) {
			foreach ( $query->tax_query->queries as $tax_query ) { //@See Query.php: 1530
				...
				// Applies saleable filters if the current taxonomy is saleable
		...
		// Applies saleable filters if the current post type is saleable
		if ( !$apply_filters && tcp_is_saleable_post_type( $query->get( 'post_type' ) ) ) {
			$apply_filters = true;
		}

		// Applies saleable filters if 'search only products' is activated 
		if ( !$apply_filters && thecartpress()->get_setting( 'search_only_products', false ) && is_search() ) {
			$apply_filters = true;
		}
		...
		if ( $apply_filters ) {
			...
			// Search page displays all items (no pagination)
			if ( is_search() ) {
	...
	function admin_menu() {
		...
		add_menu_page( '', __( 'Settings', 'tcp' ), 'tcp_edit_products', $base, '', plugins_url( 'images/tcp.png', __FILE__ ), 42 );
	...
	function activate_plugin() {
		...
		do_action( 'tcp_activate_plugin', $this );
		...
		if ( is_array( $taxonomies ) && count( $taxonomies ) > 0 ) {
			foreach( $taxonomies as $id => $taxonomy ) {
				if ( $taxonomy['activate'] ) {
					...
					// To translate, use the first post
					$post_type = $taxonomy['post_type'][0];
